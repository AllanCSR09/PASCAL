...
class NodoDAC:
    def __init__(self, id_DAC, tupla_listas, px, py):

        self.dato = id_DAC
        self.tipo = "DAC"
        self.nombre = "DAC_"+str(id_DAC)
        self.posicionx = px
        self.posiciony = py
        self.tipoNetlist = "I"
        ...
        self.imagen = PhotoImage(file="DAC.png")
        ...
        # Variables ------------------------
        self.variable_rango = 4
        self.variable_tiempo_muestreo = 20
        self.variable_ganancia = 1

    def dobleClickDAC(self, event):


        if self.ventanaCerrada:
            self.ventanaCerrada = False

            self.windowDAC = Toplevel(self.mainWindow)
            self.windowDAC.resizable(False, False)
            self.windowDAC.title(self.nombre)

            # -------------------- FRAME 1 --------------------

            self.frame1 = Frame(self.windowDAC, bg="#2C2C2C")
            self.frame2 = Frame(self.windowDAC, bg="white")
            self.frame3 = Frame(self.windowDAC, bg="white")

            self.imagen_ventana = PhotoImage(file="DACv.png")
            ...
            # -------------------- FRAME 3 --------------------
            ttk.Button(self.frame3, text="Aceptar", width=8, command=lambda: self.cerrarVentanaDAC("Aceptar"))\
                .pack(side=RIGHT, padx=(10, 20),pady=(0, 10))
            ttk.Button(self.frame3, text="Cancelar", width=8, command=lambda: self.cerrarVentanaDAC("Cancelar"))\
            ...
            self.windowDAC.protocol("WM_DELETE_WINDOW", self.cerrarVentanaDAC)

            self.windowDAC.withdraw()
            self.windowDAC.update_idletasks()
            x = (self.windowDAC.winfo_screenwidth() - self.windowDAC.winfo_reqwidth()) / 2
            y = (self.windowDAC.winfo_screenheight() - self.windowDAC.winfo_reqheight()) / 2
            self.windowDAC.geometry("+%d+%d" % (x, y))

            self.windowDAC.deiconify()
    ...
    def cerrarVentanaDAC(self, opcion="Cancelar"):
    ...
    def clickIzquierdoDAC(self, event):
        ...
        if self.borrandoElemento:
            ...
            if 38 > self.dx > 28 and 6 > abs(self.dy):  # Salida DAC
            ...
            if -38 < self.dx < -28 and 6 > abs(self.dy):  # Entrada DAC
    ...
    def clickDerechoDAC(self, event):

        self.eliminarDAC()

    def eliminarDAC(self):
        ...
        if self.creandoLinea is None:
            ...
            self.tupla_listas[13].remover(self.nombre)

    def enMovimientoDAC(self, event):
...
class ListaDAC:
    ...
    def agregar(self, tupla_listas, x, y):
        ...
        temp = NodoDAC(item, tupla_listas, x, y)
        temp.siguiente = self.cabeza
        self.cabeza = temp

        temp.window.tag_bind(temp.nombre, "<Double-Button-1>", temp.dobleClickDAC)
        temp.window.tag_bind(temp.nombre, "<ButtonRelease-1>", temp.arrastrarDAC)
        temp.window.tag_bind(temp.nombre, "<Button-1>", temp.clickIzquierdoDAC)
        temp.window.tag_bind(temp.nombre, "<B1-Motion>", temp.enMovimientoDAC)
        temp.window.tag_bind(temp.nombre, "<Button-2>", temp.clickDerechoDAC) # 3 Windows/Unix
