...
class NodoREST:
    def __init__(self, id_REST, tupla_listas, px, py):

        self.dato = id_REST
        self.tipo = "REST"
        self.nombre = "REST_"+str(id_REST)
        self.posicionx = px
        self.posiciony = py
        self.tipoNetlist = "M"
        ...
        self.imagen = PhotoImage(file="REST.png")
    ...
    def clickIzquierdoREST(self, event):
        ...
        if self.borrandoElemento:
            ...
            if 38 > self.dx > 28 and 6 > abs(self.dy):  # Salida REST
            ...
            if -38 < self.dx < -28 and -19 < self.dy < -7:  # Entrada1 REST
            ...
            if -38 < self.dx < -28 and 19 > self.dy > 7:  # Entrada2 REST
    ...
    def clickDerechoREST(self, event):

        self.eliminarREST()

    def eliminarREST(self):
        ...
        if self.creandoLinea is None:
            ...
            self.tupla_listas[20].remover(self.nombre)

    def enMovimientoREST(self, event):
        tamano_ventana_x = self.window.winfo_width() - 20
        tamano_ventana_y = self.window.winfo_height() - 20
...
# --------------------------------------------
    def senal_nombres(self, lista_nombREST):
        ...
        if self.entrada_1 is not None:
            ...
            sim_entrada1.senal_nombres(lista_nombREST)

        if self.entrada_2 is not None:
            sim_entrada2 = a.obtenerBloqueConexion(self.entrada_2, self.nombre)
            sim_entrada2.senal_nombres(lista_nombREST)

        lista_nombREST.append(self.nombre)

        return lista_nombREST
...
class ListaREST:
    ...
    def agregar(self, tupla_listas, x, y):
        ...
        temp = NodoREST(item, tupla_listas, x, y)
        temp.siguiente = self.cabeza
        self.cabeza = temp

        temp.window.tag_bind(temp.nombre, "<ButtonRelease-1>", temp.arrastrarREST)
        temp.window.tag_bind(temp.nombre, "<Button-1>", temp.clickIzquierdoREST)
        temp.window.tag_bind(temp.nombre, "<B1-Motion>", temp.enMovimientoREST)
        temp.window.tag_bind(temp.nombre, "<Button-2>", temp.clickDerechoREST) # 3 Windows/Unix
